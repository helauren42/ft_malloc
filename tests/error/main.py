...
def double_free(filename: str, expected_double_frees: int):
    out = subprocess.run(f"c++ -g3 -I ../includes ./src/tester.cpp error/{filename} test_malloc.so -Wl,-rpath,. && LD_LIBRARY_PATH=. ./a.out && /bin/rm a.out", shell=True, capture_output=True)
