...
static inline void setFirstFreeChunk(t_heap *heap,
...
inline static void addNewFreeChunk(const size_t new_payload_size,
...
static t_chunk *unfreeChunk(const size_t bytesNeeded, t_free_chunk *curr_chunk,
  ...
  const size_t extra_bytes = curr_chunk->payload_bytes - new_payload_size;
  ...
  if (split_chunk) {
    addNewFreeChunk(new_payload_size, curr_chunk, heap, prev_free, next_free,
                    extra_bytes);
  } else {
    if (!prev_free) {
  ...
  if (!curr_chunk->prev || heap->first_chunk == NULL)
    setFirstChunk(first_heap, (t_chunk *)curr_chunk);
